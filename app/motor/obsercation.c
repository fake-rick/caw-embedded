...
void motor_observation_write_pack(motor_t* motor, device_t* device) {
  ...
  // pack.body.velocity = endian_f32(as5047p_get_velocity(motor->sensor));
  if (motor->encoder_pll) {
    ...
    // pack.body.angle = endian_f32(motor->shaft_angle);
  }
  pack.body.tick = endian_u32(HAL_GetTick());
  // debug("shaft_velocity: %f", motor->shaft_velocity);
